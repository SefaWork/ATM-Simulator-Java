...
public class CreditMenu extends Menu implements ActionListener {
	...
	public CreditMenu(ATM atm) {
		super(atm);
		
		this.confirm = new JButton("Confirm");
		this.cancel = new JButton("Cancel");
		this.amount = new JSpinner();
		this.withdrawLabel = new JLabel("Type in the amount to credit. [Max $1000 Debt]");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(source == this.logoff) {
			...
			if(toWithdraw <= 0) {
				JOptionPane.showMessageDialog(this.atm, "Please put a valid amount to credit.", "Invalid Amount",JOptionPane.OK_OPTION | JOptionPane.ERROR_MESSAGE);
				this.isActive = true;
			} else {				
				int result = JOptionPane.showConfirmDialog(this.atm, "Are you sure you want to credit $" + toWithdraw + "?", "Confirm Credit", JOptionPane.YES_NO_OPTION);
				if(result == JOptionPane.YES_OPTION) {
					// Handle transaction here.
					boolean withdrawResult = this.atm.takeCredit(toWithdraw);
					if(withdrawResult) {
						JOptionPane.showMessageDialog(this.atm, "Successfully credit $" + toWithdraw + ". The credit has been added to your account.", "Success", JOptionPane.OK_OPTION | JOptionPane.INFORMATION_MESSAGE);
						this.clean();
